...
""")
    ...
    metadata_dir.mkdir()
    metadata_dir.joinpath("version.json").write_text(json.dumps({
...
def test_create_package_with_complete_structure(tmp_path: Path):
    ...
    assert (pkg_dir / "blobs" / "sha256").exists(), "blobs/sha256 directory should exist"
    
    # Verify oci-layout content
    layout_content = json.loads((pkg_dir / "oci-layout").read_text())
    assert layout_content == {"imageLayoutVersion": "1.0.0"}, "oci-layout should have correct version"
    ...
    assert len(index["manifests"]) == 1, "index.json should reference exactly one manifest"
    assert index["manifests"][0]["mediaType"] == "application/vnd.oci.image.manifest.v1+json"
...
def test_read_adp_from_package(tmp_path: Path):
    ...
    assert adp.name == "Complete Agent", f"Expected name 'Complete Agent', got '{adp.name}'"
...
def test_package_annotations(tmp_path: Path):
    ...
    if "annotations" in index["manifests"][0]:
        ...
        assert "org.opencontainers.image.title" in annotations or "io.adp.version" in annotations
...
def test_list_blobs(tmp_path: Path):
    ...
    for blob_name in blobs:
        assert isinstance(blob_name, str), f"Blob name should be string, got {type(blob_name)}"
